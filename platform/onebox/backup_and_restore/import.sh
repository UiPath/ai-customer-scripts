...
local resp_code=DEFAULT
if [ ! -z "$create_public_ml_package_version" ];
then
...
for ((i=0; i<$(echo "$public_projects" | jq -r ".data | length"); i=i+1))
do
  ...
  then
    ...
    if [ ! -z "$public_ml_package_version" ];
    then
...
then
  ...
  then
    ...
    if [ "$is_ml_package_not_public" != null ];
    then
...
if [[ -z $source_ml_package_owned_by_accountId || -z $source_ml_package_owned_by_tenantId || -z $source_ml_package_owned_by_projectId || -z $source_ml_package_id || -z $source_ml_package_version_id ]];
then
...
readonly ACCESS_TOKEN=$(cat $ML_PACKAGE_IMPORT_INPUT_FILE | jq -r 'select(.accessToken != null) | .accessToken')
