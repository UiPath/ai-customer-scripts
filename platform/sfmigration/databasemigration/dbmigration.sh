...
bcp [$DESTINATION_DB_NAME].[$DESTINATION_DB_SCHEMA].[$DestTempTableName] in $DestTempTableName.dat -n -S $DESTINATION_SERVER -U $DESTINATION_DB_USERNAME -P $DESTINATION_DB_PASSWORD -f $BASE_PATH/databasemigration/$SRC_AIC_INSTALLATION_VERSION/$DestTempTableName.fmt
...
sqlcmd -v DestinationDBSchema=$DESTINATION_PKGMANAGER_DB_SCHEMA -S $DESTINATION_SERVER -U $DESTINATION_DB_USERNAME -P $DESTINATION_DB_PASSWORD -d $DESTINATION_DB_NAME -i $BASE_PATH/databasemigration/$SRC_AIC_INSTALLATION_VERSION/create_temp_table.sql
